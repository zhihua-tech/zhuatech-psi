...
| POST | `/domain/decision` | 执行企业进销存管理系统专属领域规则 |
...
| 字段 | 类型 | 含义 |
| --- | --- | --- |
| `documentNo` | String | 业务单号 |
| `orderedQuantity` | int | 订单数量 |
| `availableQuantity` | int | 可用库存 |
| `unitPrice` | double | 销售单价 |
| `unitCost` | double | 单位成本 |
| `customerCreditAvailable` | double | 客户可用信用 |
| `shelfLifeDaysRemaining` | int | 剩余效期(天) |
| `priceApproved` | boolean | 价格已审批 |
